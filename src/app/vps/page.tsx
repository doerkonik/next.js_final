  ...
  const vpsFeatures = [
    {
      ...
      desc: 'Complete control over your server. Install any OS, software, or run any configuration you need.',
    },
    {
      icon: '⚡',
      title: 'NVMe SSD Storage',
      desc: 'Ultra-fast NVMe solid-state drives deliver 10x faster read/write speeds than traditional SSDs.',
    },
    {
      icon: '🛡️',
      title: 'DDoS Protection',
      desc: 'Enterprise-grade DDoS mitigation included in every plan at no extra cost.',
    },
    {
      icon: '🌐',
      title: '1 Gbps Network',
      desc: 'Blazing-fast 1 Gbps uplink with guaranteed network throughput for your applications.',
    },
    {
      icon: '🔄',
      title: 'Instant Provisioning',
      desc: 'Your VPS is ready in under 60 seconds after order confirmation.',
    },
    {
      icon: '📞',
      title: 'Managed Support',
      desc: 'Our Linux experts are on standby to help you configure and optimize your server.',
    },
  ]
  ...
  return (
    <div className="pt-24 pb-20">
      ...
      <Container>
        ...
        {/* VPS Plans
            - 1 col on mobile
            - 2 col on sm (better than squishing 4 cards)
            - 4 col on lg and above
        */}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 mb-16 sm:mb-20">
          {vpsPlans.map((p) => (
            ...
            >
              {p.popular && (
                <div className="absolute -top-3 left-1/2 -translate-x-1/2 bg-white text-primary text-[10px] font-black px-3 py-1 rounded-full shadow-lg whitespace-nowrap">
                  BEST VALUE
                </div>
              )}
              <div
                className={`text-xs font-bold mb-2 uppercase tracking-wider ${p.popular ? 'text-white/80' : 'text-primary'
                  }`}
              >
                {p.name}
              </div>
              <div className="flex items-baseline gap-1 mb-4 flex-wrap">
                <span
                  className={`text-3xl font-extrabold ${p.popular ? 'text-white' : 'text-slate-800 dark:text-white'
                    }`}
                >
                  ৳{p.price}
                </span>
                <span className={`text-xs ${p.popular ? 'text-white/60' : 'text-slate-400'}`}>
                  /mo
                </span>
              </div>
              <div className="space-y-2 mb-6">
                {[
                  ...
                  <div
                    key={spec.label}
                    className={`flex items-center gap-2 text-xs ${p.popular ? 'text-white/90' : 'text-slate-600 dark:text-slate-300'
                      }`}
                  >
        ...
        {/* min-w ensures the table never squishes below a readable width on mobile */}
        <div className="glass-card rounded-2xl overflow-hidden border-glow mb-16">
          <div className="overflow-x-auto">
            <table className="w-full min-w-[560px]">
              <thead>
                <tr className="bg-primary/10 border-b border-primary/20">
                  <th className="text-left p-4 text-sm font-bold text-slate-700 dark:text-slate-300 w-1/3">
                    Specification
                  </th>
                  {vpsPlans.map((p) => (
                    <th
                      key={p.name}
                      className={`p-4 text-center text-sm font-bold ${p.popular ? 'text-primary' : 'text-slate-600 dark:text-slate-400'
                        }`}
                    >
                      {p.name}
                    </th>
              ...
              <tbody>
                {specRows.map((row, i) => (
                  <tr
                    key={row.label}
                    className={`border-b border-white/10 last:border-0 ${i % 2 === 0 ? 'bg-white/20 dark:bg-white/[0.02]' : ''
                      }`}
                  >
                    ...
                    {row.values.map((v, j) => (
                      <td
                        key={j}
                        className="p-4 text-center text-sm font-semibold text-slate-700 dark:text-slate-300"
                      >
                        {v}
                      </td>
                    ))}
        ...
        {/* Contact CTA */}
        <Section>
          <div className="glass-card rounded-3xl p-8 sm:p-10 text-center border-glow">
            <h2 className="text-xl sm:text-2xl font-extrabold text-slate-800 dark:text-white mb-3">
              Ready to Order?
            </h2>
            <p className="text-slate-500 dark:text-slate-400 mb-6 text-sm max-w-md mx-auto">
              VPS orders are handled manually. Contact our support team via WhatsApp or email to get
              started instantly.
            </p>
            <div className="flex justify-center gap-4 flex-wrap">
              <a
                href="https://wa.me/8801810176823"
                className="px-8 py-3 rounded-xl bg-green-500 text-white font-bold hover:scale-105 transition-all text-sm glow-primary"
              >
                💬 WhatsApp Us
              </a>
              <a
                href="mailto:dtech@doer.com.bd"
                className="px-8 py-3 rounded-xl glass border border-primary/30 text-primary font-bold hover:bg-primary/10 transition-all text-sm"
              >
                ✉️ Email Support
              </a>
            </div>
