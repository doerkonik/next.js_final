...
export default function HostingPage() {
  ...
  const stacks = [
    'Node.js', 'Python', 'PHP 8.x', '.NET Core', 'Next.js',
    'React', 'Vue.js', 'Laravel', 'Django', 'Ruby on Rails',
    'WordPress', 'MySQL', 'PostgreSQL', 'Redis',
  ]
  ...
  return (
    <div className="pt-24 pb-20">
      ...
      <Container>
        ...
        {/* Plans — single column on mobile, 3-col from sm */}
        ...
        {/* overflow-x-auto on a wrapper so the table scrolls horizontally on small screens
            instead of breaking the page layout */}
        <div className="glass-card rounded-2xl border-glow overflow-hidden mb-16">
          <div className="overflow-x-auto">
            <table className="w-full min-w-[480px]">
              <thead>
                <tr className="border-b border-white/20 dark:border-white/10">
                  <th className="text-left p-4 sm:p-5 text-sm font-bold text-slate-600 dark:text-slate-400 w-2/5">
                    Feature
                  </th>
                  {hostingPlans.map((p) => (
                    <th
                      key={p.name}
                      className={`p-4 sm:p-5 text-center text-sm font-bold ${p.popular ? 'text-primary' : 'text-slate-600 dark:text-slate-400'
                        }`}
                    >
                      {p.name}
                      {p.popular && (
              ...
              <tbody>
                {comparisonRows.map((row, i) => (
                  ...
                  >
                    <td className="p-4 sm:p-5 text-sm text-slate-600 dark:text-slate-400 font-medium">
                      {row.label}
                    </td>
                    {row.values.map((v, j) => (
                      <td key={j} className="p-4 sm:p-5 text-center text-sm font-semibold">
                        {v === true ? (
        ...
        <Section>
          <SectionHeader
            title="Supported"
            highlight="Tech Stacks"
            subtitle="Deploy anything. We've got you covered."
          />
          <div className="flex flex-wrap justify-center gap-3 sm:gap-4">
            {stacks.map((s) => (
              <div
                key={s}
                className="glass-card rounded-xl px-5 py-2.5 text-sm font-bold text-slate-700 dark:text-slate-300 border-glow hover:text-primary hover:border-primary/50 transition-all cursor-default"
              >
                {s}
              </div>
            ))}
