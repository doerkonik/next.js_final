...
const domainFeatures = [
  { icon: '⚡', title: 'Instant Activation', desc: 'Your domain gets activated immediately after completing the registration process.' },
  { icon: '🔒', title: 'Secure & Trusted', desc: 'We are BTCL accredited and ensure safe, reliable domain registration with every order.' },
  { icon: '🇧🇩', title: 'National Identity', desc: 'Establish your digital presence with a .BD domain and showcase your Bangladeshi identity online.' },
  { icon: '🕐', title: '24/7 Support', desc: 'Our dedicated support team is always ready to help you with any queries in no time.' },
]
      ...
      <Container>
        ...
        <div className="max-w-2xl mx-auto mb-16">
          <div className="glass-card rounded-2xl p-2 border-glow">
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 px-5 py-4 bg-transparent text-slate-800 dark:text-white placeholder-slate-400 focus:outline-none font-medium text-sm"
                disabled={status === 'loading'}
              />
              <button
                onClick={() => search()}
                disabled={status === 'loading' || !query.trim()}
                className="px-8 py-4 rounded-xl bg-primary text-white font-bold glow-primary hover:scale-105 active:scale-95 transition-all text-sm whitespace-nowrap disabled:opacity-50 disabled:cursor-not-allowed disabled:scale-100"
              >
                {status === 'loading' ? (
          ...
          {status === 'available' && result && (
            <div className="mt-4 glass-card rounded-2xl px-6 py-4 border border-green-400/30 bg-green-50/10 flex items-center justify-between">
              <div>
                <p className="text-green-400 font-bold text-sm">✅ Available!</p>
                <p className="text-slate-700 dark:text-white font-extrabold text-lg">{result.domain}</p>
              </div>
              <button className="px-5 py-2.5 rounded-xl bg-green-500 text-white text-sm font-bold hover:bg-green-600 transition-all">
                Register Now
              </button>
            </div>
          )}
          ...
          {status === 'taken' && result && (
            <div className="mt-4 glass-card rounded-2xl px-6 py-4 border border-red-400/30 bg-red-50/10">
              <p className="text-red-400 font-bold text-sm">❌ Already Taken</p>
              <p className="text-slate-700 dark:text-white font-semibold">
                <span className="font-extrabold">{result.domain}</span> is not available. Try a different name or extension below.
              </p>
            </div>
          )}

          {status === 'error' && (
            <div className="mt-4 glass-card rounded-2xl px-6 py-4 border border-amber-400/30 bg-amber-50/10">
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
              {domains.map((domain) => (
                ...
                >
                  ...
                  <div className="text-2xl font-extrabold text-slate-800 dark:text-white mb-1 group-hover:text-primary transition-colors">
                    {domain.extension}
                  </div>
                  <div className="text-sm text-slate-500 dark:text-slate-400 mb-4">{domain.description}</div>
                  <div className="flex items-center justify-between">
                    ...
                    <button
                      ...
                      className="px-4 py-2 rounded-xl bg-primary/10 text-primary text-xs font-bold border border-primary/20 hover:bg-primary hover:text-white transition-all"
                    >
                      Register
                    </button>
                  </div>
